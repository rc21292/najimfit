...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            if(Auth::guard('web')->check()){
                $fetch = Client::where('id', $request['id'])->first();
            }elseif(Auth::guard('api')->check()){
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('clients',  function ($join) {
            $join->on('messages.from_id', '=', 'clients.id')
            ->orOn('messages.to_id', '=', 'clients.id');
        })
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                    ...
                    // Get user data
                    $userCollection = Client::where('id', $user->id)->first();
    ...
    public function search(Request $request)
    {
        ...
        if(Auth::guard('web')->check()){
            $records = Client::where(DB::raw("CONCAT(`firstname`, ' ', `lastname`)"),'LIKE',"%{$input}%");
        }elseif(Auth::guard('api')->check()){
        ...
        // return Response::json(['success'=>$records->get()]);
        // send the response
        return Response::json([
