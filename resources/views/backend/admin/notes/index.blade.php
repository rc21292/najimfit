...
<script>
	...
	[ "{{ $no++ }}" ,"{{ $request->nutritionist_name }}"," <a href='{{route('client-full-profile.show',$request->client_id)}}' >{{ $request->client_name}}</a><br>ID: {{ $request->client_id}}","{{ $request->note }}", "<a href='javascript:' data-toggle='modal' data-target='#myModal' data-client='{{$request->client_id}}' data-id='{{$request->id}}' class='btn btn-danger btnpro'>Actions</a>"],
	@endforeach
	];
