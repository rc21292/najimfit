    ...
    <div class="col-md-12">
        <div class="ms-panel">
            <div class="ms-panel-header">
                <h6> Nutritionist with  Client Chats</h6>
            </div>
...
<script>
    ...
    [
    "{{ $no++}}" ,"<a href='{{route('chat.show',$client->client_id)}}'><img src='https://via.placeholder.com/216x62' style='width:50px; height:30px;'> {{ $client->firstname }} {{ $client->lastname}}</a><p style='margin-left:40px;'>ID: {{ $client->client_id }}</p>", "{{{ $client->lables}}}", "{{{ $client->chat_status }}}", "<a href='{{route('client-full-profile.show',$client->client_id)}}' class='btn btn-primary btnpro'>Profile</a><a href='{{route('labels.show',$client->client_id)}}'class='btn btn-primary btnpro'>Labels</a> @if($client->is_deferd)<a href='{{route('requests.index')}}' class='btn btn-info btnpro'>Defer</a> @else <a href='{{route('defer',$client->client_id)}}' class='btn btn-info btnpro'>Defer</a> @endif @if($client->is_complaint)<a href='{{route('complaints.index')}}' class='btn btn-info btnpro'>Post Complaint</a> @else <a href='{{route('post-complaint',$client->client_id)}}' class='btn btn-info btnpro'>Post Complaint</a> @endif ","{{ $client->assigned_on}}<br>{{ $client->name }}"],
    @endforeach
    ];
