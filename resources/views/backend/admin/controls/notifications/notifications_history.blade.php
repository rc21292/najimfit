	...
	<div class="col-md-12">
		<nav aria-label="breadcrumb " class="ms-panel-custom">
			<ol class="breadcrumb pl-0">
				...
				<li class="breadcrumb-item active" aria-current="page">Notifications History</li>
			</ol>
...
<script>
	var dataSet18 = [
	@foreach($notifications_history as $notification_history)
	[ "{{ $no++ }}" ,"{{ $notification_history->firstname }} {{ $notification_history->lastname}}"," {{ $notification_history->message }}"," {{ $notification_history->message_type }}", @if($notification_history->status)"Sended" @else "Failed" @endif, @if($notification_history->from_subscriptions)"From Subscription" @else "From Notification" @endif],
	@endforeach
	];
