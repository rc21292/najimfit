...
<script>
	...
	[ "{{ $no++ }}" ,"{{ $client->firstname }} {{ $client->lastname}}"," {{ $client->email }}"," {{ $client->phone }}", @if($client->is_subscription_canceled) "<a href='{{route('uncancel-client-subscription',$client->id)}}' class='btn btn-primary btnpro'>Uncancel Subscription</a>" @else "<a href='{{route('cancel-client-subscription',$client->id)}}' class='btn btn-primary btnpro'>Cancel Subscription</a>" @endif],
	@endforeach
	];
