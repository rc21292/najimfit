...
<script>
	...
	[ "{{ $no++ }}" ,"{{ $client->firstname }} {{ $client->lastname}}"," {{ $client->email }}"," {{ $client->phone }}", @if($client->blocked_from_app) "<a href='{{route('unblock-user-from-app',$client->id)}}' class='btn btn-danger btnpro'>Unblock Client</a>" @else "<a href='{{route('block-user-from-app',$client->id)}}' class='btn btn-primary btnpro'>Block Client</a>" @endif],
	@endforeach
	];
