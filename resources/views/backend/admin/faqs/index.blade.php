...
<div class="row">
	<div class="col-md-12">
		<nav aria-label="breadcrumb " class="ms-panel-custom">
			...
			<a href="{{route('faqs.create')}}" class="ms-btn-icon btn-square btn-secondary"><i class="fas fa-plus"></i></a>
		</nav>
...
<script>
	var dataSet18 = [
	@foreach($faqs as $faq)
	[ "{{ $no++ }}" , "{{ $faq->question }}" , "{{ $faq->answer }}", @if($faq->status == '1')"Enabled" @else "Disabled" @endif , "<a href='{{route('faqs.edit',$faq->id)}}'><i class='fas fa-pencil-alt ms-text-primary'></i></a> <a href='javascript:' onclick='submitform({{ $no }});'><i class='far fa-trash-alt ms-text-danger'></i></a><form id='delete-form{{$no}}' action='{{route('faqs.destroy',$faq->id)}}' method='POST'><input type='hidden' name='_token' value='{{ csrf_token()}}'><input type='hidden' name='_method' value='DELETE'></form>"],
	@endforeach
	];
