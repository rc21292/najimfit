...
class AuthController extends Controller
{
	...
	, $request->mobile
		...
		if ($no_exists) 
		{
			...
			$token = "e0b250480f9a89ad3dd670e488f6d1f7";
	...
	public function getuserbodyprofile(Request $request){
		...
		if($request->language == 'arabic'){
			...
			$questions_body_size = DB::table('client_answers')->join('questions','questions.id','=','client_answers.question_id')->select('questions.id as question_id','questions.question_arabic','questions.question_trype','client_answers.id as answer_id' , 'client_answers.answer')->where('client_answers.client_id', $user_id)->where('questions.category',2)->get();

			$questions_health = DB::table('client_answers')->join('questions','questions.id','=','client_answers.question_id')->select('questions.id as question_id','questions.question_arabic','questions.question_trype','client_answers.id as answer_id' , 'client_answers.answer')->where('client_answers.client_id', $user_id)->where('questions.category',3)->get();

			$questions_medicine = DB::table('client_answers')->join('questions','questions.id','=','client_answers.question_id')->select('questions.id as question_id','questions.question_arabic','questions.question_trype','client_answers.id as answer_id' , 'client_answers.answer')->where('client_answers.client_id', $user_id)->where('questions.category',5)->get();
		}else{
