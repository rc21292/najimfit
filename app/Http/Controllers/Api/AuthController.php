...
class AuthController extends Controller
{
	...
	public function getuserbodyprofile(Request $request){
		...
		if($request->language == 'arabic'){
			...
			$questions_health = DB::table('client_answers')->join('questions','questions.id','=','client_answers.question_id')->select('questions.id as question_id','questions.question','questions.question_type','client_answers.id as answer_id' , 'client_answers.answer')->where('client_answers.client_id', $user_id)->where('questions.hint','!=','Medication name')->where('questions.category',3)->get();
